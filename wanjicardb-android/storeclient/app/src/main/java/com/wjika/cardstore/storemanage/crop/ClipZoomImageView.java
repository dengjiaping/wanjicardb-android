...
public class ClipZoomImageView extends ImageView implements
	...
	public void onGlobalLayout() {
		if (once) {
			...
			//因为现在图片裁剪框是居顶部，所以图片小于View的高度时，会距离顶部有个间距，将图片上移这个间距就可以了
			if (getMatrixRectF().top > 0) {
				mScaleMatrix.postTranslate(0, -getMatrixRectF().top);
			}
	...
	public Bitmap clip() {
		Bitmap bitmap;
		try {
			bitmap = Bitmap.createBitmap(getWidth(), getHeight() - getHVerticalPadding(),
			...
			int y = (int) TypedValue.applyDimension(
					TypedValue.COMPLEX_UNIT_DIP, 1, getResources()
							.getDisplayMetrics());
			Bitmap tempBitmap = Bitmap.createBitmap(bitmap, mHorizontalPadding,
					y, getWidth() - 2 * mHorizontalPadding,
	...
	private void checkBorder() {
		...
		// 如果宽或高大于屏幕，则控制范围; 这里.001是因为精度丢失会产生问题，但是误差一般很小，我们直接加了0.01
		if (rect.width() + 0.01 >= width - 2 * mHorizontalPadding) {
			//如果图片宽度大于裁剪框的宽度，设置最左和最右边能移动到的位置
			if (rect.left > mHorizontalPadding) {
				//如果图片距离左边的间距大于一边，就设置图片最左的位置，达到限制左边移动的效果，后面的都差不多
		...
		/*rect.top是图片上端距离裁剪框顶部的距离,rect.bottom则相反
		rect.height()是图片的高度，height是除头布局外的整个布局的高度
		getHVerticalPadding是上下内间距，实际是下间距应该是(getHVerticalPadding * 2)*/
		if (rect.height() + 0.01 >= height - 2 * getHVerticalPadding()) {
			if (rect.top > 0) {
				deltaY = -rect.top;
			}
			if (rect.bottom < height - getHVerticalPadding() * 2) {
				deltaY = height - getHVerticalPadding() * 2 - rect.bottom;
			}
		}
